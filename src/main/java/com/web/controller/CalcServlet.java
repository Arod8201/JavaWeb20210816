...
/*
 寫一個 CalcServlet 當
 瀏覽器輸入 : http://localhost:8000/JavaWeb20210816/controller/calc?h=170&w=60
 HTTP 方法 : GET
 網頁可以得到 : 20.76
*/
	...
	private void doHandler(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (h != null && w != null) {
			try {
				...
				out.printf(String.format("h: %.1f w: %.1f bmi: %.2f", double_h, double_w, bmi));
			} catch (Exception e) {
				// 轉換失敗
				out.print("Args is not numeric : " + e);
			}
		} else {
			// 無設定參數
